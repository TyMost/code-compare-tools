...
`examples/` 目录用于演示 `rules.yaml` 中的迁移标签如何在真实代码块上命中。目录下维护一对源目标仓库：

- `projectA/`：迁移前的基线工程，保留了 `LEGACY::` 前缀、`token.substring` 等旧实现。
- `projectB/`：迁移后的目标工程，应用了 `encoder.encode / encoder.decodeTenant`、`override` 等新实现。
...
1. 运行 `examples/init-git-history.sh` 可为两个项目写入演示用的 Git 历史。
2. 调整规则后，先更新 `manifest.yaml` 中的行数与占比，再运行后端扫描以核对标签命中情况。
3. 新增示例时，请确保：
   - 同名文件在 `projectA/` 与 `projectB/` 下均存在（未迁移的例外）。
   - 将新增代码块归入上述分类之一，并更新 `manifest.yaml` 与本 README。
4. 如果需要扩展“其他”占比，可在 `integration/` 或 `innovation/` 目录添加保持最小改动的样例。
